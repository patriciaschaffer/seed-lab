...
"""
    ...
    # Enhanced revision examples for precision editing
    revision_examples = [
        ("Troque 'because' por 'and that' neste texto", "Aqui está o texto com a alteração solicitada:"),
        ("Change 'very good' to 'excellent'", "Here's the text with your requested modification:"),
        ("Substitua 'problema' por 'desafio'", "Texto corrigido conforme solicitado:"),
        ("Replace this word with that one", "Modified text as requested:"),
    ]

    selected_examples = random.sample(example_conversations, min(3, len(example_conversations)))
    
    # Add revision examples if this looks like a revision request
    is_revision = detect_revision_mode(user_input)
    if is_revision:
        selected_examples.extend(random.sample(revision_examples, min(2, len(revision_examples))))
    examples_text = ""
...
def detect_revision_mode(user_input):
    """Detect if user is requesting specific text changes"""
    revision_keywords = [
        "trocar", "mudar", "alterar", "substituir", "corrigir",
        "change", "replace", "modify", "fix", "switch",
        "em vez de", "instead of", "rather than"
    ]
    return any(keyword in user_input.lower() for keyword in revision_keywords)

def generate_lia_response(user_input, llm, max_length=200):  # Increased tokens
    ...
    is_revision = detect_revision_mode(user_input)
    if is_revision:
        max_length = 250  # More space for revisions
        temp = 0.4  # Lower temperature for precision
    else:
        temp = 0.6
