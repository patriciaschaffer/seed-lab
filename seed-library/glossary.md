...
A field of AI research and practice focused on developing models whose internal workings, decisions, and outputs can be understood and interpreted by humans. XAI aims to make AI behavior transparent, understandable, and accountable, so users can trust and validate model predictions. It encompasses methods for both post-hoc explanation (explaining existing models) and intrinsic interpretability (designing inherently understandable models).

---

### Interpretability

The degree to which a human can understand the cause of a decision or prediction made by an AI system. Interpretability can refer to:
global interpretability: understanding the model’s overall logic or structure; local interpretability: understanding why a model produced a specific output for a given input. High interpretability enables users to anticipate model behavior, identify errors or biases, and build trust in AI systems.
